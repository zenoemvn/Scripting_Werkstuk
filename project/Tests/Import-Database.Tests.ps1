BeforeAll {
    # Import modules
    Import-Module "$PSScriptRoot\..\Modules\Migration\Import-DatabaseFromCsv.psm1" -Force
    Import-Module "$PSScriptRoot\..\Modules\Migration\Import-CsvToSqlTable.psm1" -Force
    
    # Test configuratie
    $script:ServerInstance = "localhost\SQLEXPRESS"
    $script:TestDatabase = "PesterTestDB_$(Get-Random)"
    $script:CsvFolder = "$PSScriptRoot\..\Output"
...
"@
        ...
        It "Should successfully join OrderDetails with Products" {
            $result = Invoke-Sqlcmd -ServerInstance $script:ServerInstance -Database $script:TestDatabase -TrustServerCertificate -Query @"
SELECT od.OrderDetailID, p.ProductName
FROM OrderDetails od
